...
if ($EnableEmail) {
  [string]$emailFooter = "`n`nThis email was generated by $(Split-Path -Path $PSCommandPath -Leaf) at $((Get-Date).ToString('dd/MM/yyyy HH:mm:ss')) on $(([System.Net.Dns]::GetHostByName(($env:computerName))).Hostname)." + `
