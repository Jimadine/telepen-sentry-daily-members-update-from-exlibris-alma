...
Test-Paths -LogFilePath $LogFilePath -ApiKeysDirectoryPath $ApiKeysDirectoryPath -OutputFileDirectoryPath $OutputFileDirectoryPath
$strApiKey = Get-ApiKey -ApiKeysDirectoryPath $ApiKeysDirectoryPath
$BaseUrl = $BaseUrl -replace 'api-[^\\.]+', "api-$ApiRegion"
$strUrl = '{0}{1}?path={2}&limit={3}' -f $BaseUrl, $BasePath, [System.Uri]::EscapeDataString($ReportPath), $RowLimit
Get-Data -StrUrl $strUrl `
  -StrApiKey $strApiKey `
  -OutputFileDirectoryPath $OutputFileDirectoryPath `
