...
if ($EnableEmail) {
  [string]$emailFooter = "`n`nThis email was generated by $(Split-Path -Path $PSCommandPath -Leaf) at $((Get-Date).ToString('dd/MM/yyyy HH:mm:ss')) on $(([System.Net.Dns]::GetHostByName(($env:computerName))).Hostname).`nFor more information on this email, see https://uoy.atlassian.net/wiki/spaces/ittechdocs/pages/43389721/FMSYS+Library+-+Sentry+turnstile+software"
}
  ...
  try {
    ...
    if ($objRestReq.StatusCode -eq 200) {
      ...
      elseif ($objRemoteError) {
        if ($EnableEmail) {
          Send-MailMessage -From $EmailSender -To $($EmailRecipient.Split(',') | ForEach-Object { $_.Trim() }) -SmtpServer $EmailSmtp -Subject "${EmailSubjectPrefix} XML response error" -Body "Total rows written: ${rowCount}`nError description: ${objRemoteError}${emailFooter}"
        }
      ...
      if ($EnableEmail) {
        Send-MailMessage -From $EmailSender -To $($EmailRecipient.Split(',') | ForEach-Object { $_.Trim() }) -SmtpServer $EmailSmtp -Subject "${EmailSubjectPrefix} Unexpected HTTP response code" -Body "Total rows written: ${rowCount}`nError description: $(objRestReq.StatusText)${emailFooter}"
      }
    ...
    if ($retryCount -eq $RetryAttempts) {
      if ($EnableEmail) {
        Send-MailMessage -From $EmailSender -To $($EmailRecipient.Split(',') | ForEach-Object { $_.Trim() }) -SmtpServer $EmailSmtp -Subject "${EmailSubjectPrefix} $($_.Exception.GetType().Name)" -Body "Total rows written: ${rowCount}`nError description: $($_.Exception.Message)${emailFooter}"
      }
...
if ($rowCount -lt $ProblemRowCount -and $success -eq $true) {
  if ($EnableEmail) {
    Send-MailMessage -From $EmailSender -To $($EmailRecipient.Split(',') | ForEach-Object { $_.Trim() }) -SmtpServer $EmailSmtp -Subject "${EmailSubjectPrefix} Rows written report" -Body "Total rows written: ${rowCount}${emailFooter}"
  }
